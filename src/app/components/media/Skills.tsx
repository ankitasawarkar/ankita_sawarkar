...
interface SkillsProps {
    skills: string[];
    platform?: string; // Make platform an optional prop
}

const Skills: React.FC<SkillsProps> = ({ skills, platform }) => {
    return (
        <div className="flex  flex-wrap space-x-3" >
            {skills.map((skill, index) => (
                <div key={`${platform || "default"}-${skill}-${index}`} className="relative group">
